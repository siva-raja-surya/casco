...
export const UserWizard = ({ onAdminAccess, onLogout, onUserVerified }) => {
  ...
  // Specific error state for live validation
  const [utrError, setUtrError] = useState('');
  const [mblError, setMblError] = useState('');
  ...
  const validateAlphanumericFormat = (value) => {
  ...
  const validateUTR = (value) => {
    if (!value) return false;
    const segments = value.split('/').map(s => s.trim());
    // 10-22 digits, alphanumeric
    const regex = /^[a-zA-Z0-9]{10,22}$/;
    
    for (const seg of segments) {
      // If user types a slash, the next segment might be empty momentarily. 
      // Validation will fail for empty string, which correctly prompts user to finish typing.
      if (!regex.test(seg)) {
        return false;
      }
    }
  ...
  const handleUTRChange = (e) => {
    ...
    setFormData(prev => ({ ...prev, utrNo: value }));
    
    // Live Validation logic
    if (value === '') {
      setUtrError('');
    } else if (!validateUTR(value)) {
      setUtrError('Each UTR must be 10-22 alphanumeric characters.');
    } else {
      setUtrError('');
    }
  };
  ...
  const handleMBLChange = (e) => {
    const value = e.target.value;
    setFormData(prev => ({ ...prev, mblNo: value }));

    if (value === '') {
      setMblError('');
    } else if (!validateAlphanumericFormat(value)) {
      setMblError('Must be alphanumeric (A-Z, 0-9). Separate multiple with /');
    } else {
      setMblError('');
    }
  };
  ...
  const handleInvoiceChange = (e) => {
    ...
    if (value === '') {
      ...
      setInvoiceError('Must be alphanumeric (A-Z, 0-9). Separate multiple with /');
    } else {
      setInvoiceError('');
    }
  };
  ...
  const handleSubmitForm = async (e) => {
    ...
    // 2. Validate UTR (10-22 chars, alphanumeric, / separator)
    if (!validateUTR(formData.utrNo)) {
      setUtrError("Each UTR must be 10-22 alphanumeric characters.");
      alert("Please correct the UTR No.");
      return;
    }

    // 3. Export Case Specifics
    if (formData.caseType === 'Export') {
      if (!formData.mblNo) {
         alert("Please fill in MBL NO for Export cases.");
         return;
      }
      if (!validateAlphanumericFormat(formData.mblNo)) {
        setMblError("Must be alphanumeric.");
        alert("Please correct the MBL NO.");
        return;
      }
    }

    // 4. Invoice Validation (if visible and entered)
    if (formData.caseType !== 'Security Deposit') {
       if (formData.invoiceNo && !validateAlphanumericFormat(formData.invoiceNo)) {
         setInvoiceError("Must be alphanumeric.");
         alert("Please correct the Invoice No.");
         return;
       }
    }
  ...
  const showMBL = formData.caseType === 'Export';
  const showInvoice = formData.caseType !== 'Security Deposit';
  
  // Logic for dynamic numbering
  // 1-5 are fixed.
  // MBL is 6 if visible.
  // Invoice is 6 or 7 depending on MBL.
  const mblLabelNumber = 6;
  const invoiceLabelNumber = showMBL ? 7 : 6;
      ...
      <div className="w-full max-w-2xl bg-white rounded-lg shadow-lg overflow-hidden border-t-4 border-cosco-primary">
        <div className="p-8 md:p-10">
          ...
          <Input 
            ...
            error={error}
            helperText="Check your server logs for the OTP if using local backend."
          />
        ...
        <div className="bg-white p-8 md:p-10">
          ...
          <form onSubmit={handleSubmitForm} className="space-y-6">
            ...
            <Input 
              ...
              readOnly
              disabled
              className="bg-gray-200 cursor-not-allowed pointer-events-none" 
              required
            ...
            <Input 
              ...
              required
              error={utrError}
              helperText={!utrError ? "Must be 10-22 alphanumeric digits." : undefined}
            />
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {['Export', 'Import', 'Others', 'Security Deposit'].map((opt) => (
                  <div key={opt} className="flex items-center">
                    <input
                      ...
                      onChange={() => setFormData({...formData, caseType: opt})}
                    />
            ...
            {showMBL && (
              <Input 
                label={`${mblLabelNumber}. MBL NO (If Multiple MBL NO, please separate with forward slash ( / ) and Mention only numeric number)`}
                placeholder="Enter your answer"
                name="mblNo"
            ...
            {showInvoice && (
              <Input 
                label={`${invoiceLabelNumber}. Invoice No (If Multiple Invoice No, please separate with forward slash ( / )`}
                placeholder="Enter your answer"
                name="invoiceNo"
    ...
    return (
      <div className="w-full max-w-lg bg-white rounded-lg shadow-lg overflow-hidden border-t-4 border-cosco-primary p-12 text-center animate-fade-in-up">
        ...
        <div className="bg-gray-50 rounded p-4 mb-6 text-left text-sm">
          ...
          {formData.mblNo && <p><strong>MBL NO:</strong> {formData.mblNo}</p>}
        </div>
